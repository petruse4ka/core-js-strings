...
function repeatString(str, times) {
  const string = times > 0 ? str.repeat(times) : '';
  return string;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    ...
    for (let j = 0; j < array[i].length; j += 1) {
      reversedWord = array[i][j] + reversedWord;
    }
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    } else if (letter === str[i].toUpperCase()) {
...
function extractNameFromTemplate(value) {
  const result = value.replace(/[^a-z0-9 ]/gi, '').replace('Hello ', '');
  return result;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const baseLetter = str.charAt(i);
    const index = latin.indexOf(baseLetter);
    const rotLetter = index !== -1 ? rot13.charAt(index) : baseLetter;
    result += rotLetter;
  }
...
function getCardId(value) {
  ...
  const result = cardDeck.indexOf(value);
  return result;
}
